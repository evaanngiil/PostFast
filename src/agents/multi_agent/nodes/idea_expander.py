...
def run_idea_expander_node(state: AgentState) -> Dict[str, Any]:
    ...
    llm = ChatGoogleGenerativeAI(model=MEDIUM_LLM, google_api_key=GENAI_API_KEY, temperature=0.7)
    ...
    # Normalización del payload de salida estructurado (Pydantic a dict)
    if hasattr(expanded_idea_object, 'dict'):
        expanded_idea_dict = expanded_idea_object.dict()
    elif hasattr(expanded_idea_object, 'model_dump'):
        expanded_idea_dict = expanded_idea_object.model_dump()
    elif isinstance(expanded_idea_object, dict):
        expanded_idea_dict = expanded_idea_object
    else:
        expanded_idea_dict = dict(expanded_idea_object)
    
    return {"fleshed_out_idea": expanded_idea_dict}
